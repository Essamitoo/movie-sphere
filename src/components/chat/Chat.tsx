...
const Chat: React.FC<ChatProps> = ({ socket, username, room, photo, rol,cover, leaveRoom }) => {
  ...
  scrollToBottom();
    ...
    useEffect(() => {
        ...
        const handleUserLeft = (data: { username: string }) => {
            ...
            setListMenssage((list) => [...list, { author: data.username, message: "", room: "", photo: "", time: "", rol: "" }]);
        };
    ...
    useEffect(():any => {
        const messageHandle = (data: Message) => { setListMenssage((list) => [...list, data]); };
    ...
    return (
        ...
        <div className="mx-auto w-full max-w-6xl rounded-2xl overflow-hidden absolute z-2 top-25 left-10">
          ...
          <div className="grid grid-cols-[70%_30%]">
            {/* Chat messages */}
            <div className="border border-gray-100/30 bg-[#171717]/60 h-[70vh] overflow-y-auto flex flex-col p-4 gap-3" ref={messagesContainerRef}>
                  ...
                  <div
                    ...
                    className={`${
                      username === item.author
                        ? "bg-[#202020] border border-[#00A878] text-white"
                  ...
                  >
                    <div className="flex justify-between w-full text-xs">
                      <img
                        src={item.photo}
                        alt=""
                      className={`w-12 h-12 rounded-full border-2 p-1 ${item.rol === "Premium"
                        ? "border-yellow-300"
                        : item.rol === "Admin"
                        ? "border-blue-700"
                        : ""}`}
                    ...
                    <p className="mt-1  text-center font-medium text-xl">{item.message}</p>
                  </div>
                ) : (
            ...
            {/* User list */}
            <div className="flex flex-col border border-gray-100/30 bg-[#171717]/70 p-2">
              {users.length > 0 ? (
                users.map((item, index) => (
                  <div key={index} className="flex items-center gap-2 text-[#00A878] mb-2">
                    <img
                      src={item.image || "default-image.jpg"}
                      alt=""
                      className={`w-15 h-15 rounded-full border-2 p-1 ${item.rol === "Premium"
                        ? "border-yellow-300"
                        : item.rol === "Admin"
                        ? "border-blue-700"
                        : ""}`}
...
>
          ...
          {/* Footer */}
          <div className="grid grid-cols-[60%_20%] border border-gray-400 rounded-b-2xl p-4 justify-center bg-[#171717]">
            <input
              type="text"
              placeholder="Mensaje..."
            ...
            <div className="flex flex-col items-center justify-between gap-1">
              {/*rol === "Premium" ? (
                <button className="bg-purple-500 text-white px-2 py-1 rounded">VIP</button>
              ) : (
                <button className="bg-gray-500 text-white px-2 py-1 rounded">Free</button>
              )*/}
