		...
		<div className='bg-[#202020] max-h-max min-h-lvh pb-5 text-white'>
			...
			<div className='flex gap-4 justify-around'>
				<div className='flex justify-around w-[20%]'>
					...
					>
						Series
						{filter === 'serie' && <div className='bg-[#00A878] h-1'></div>}
					</button>
				</div>
				<div className='flex justify-around w-[50%]'>
					<details
						className='flex flex-col w-[150px]'
						open={openDetails === 'year'}
					>
						<summary
							className={`cursor-pointer ${yearFilter && 'text-[#00A878]'}`}
							onClick={() => setOpenDetails('year')}
						>
							{yearFilter === '' ? 'Año de estreno' : yearFilter}
						</summary>
						{openDetails === 'year' && (
							<div className='flex flex-col absolute bg-[#202020] z-4 w-[150px]'>
								<button
									onClick={() => {
										setYearFilter('2024')
										setOpenDetails(null)
					...
					<details
						className='flex flex-col w-[150px]'
						open={openDetails === 'genre'}
					>
						<summary
							className={`cursor-pointer ${genreFilter && 'text-[#00A878]'}`}
							onClick={() => setOpenDetails('genre')}
						>
							{genreFilter === '' ? 'Genero' : genreFilter}
						</summary>
						{openDetails === 'genre' && (
							<div className='flex absolute flex-col bg-[#202020] z-4 w-[150px]'>
								<button
									onClick={() => {
					...
					<details className='z-4 w-[180px]' open={openDetails === 'age'}>
						<summary
							className={`cursor-pointer ${ageFilter && 'text-[#00A878]'}`}
							onClick={() => setOpenDetails('age')}
						>
							{ageFilter === '' ? 'Clasificación por Edad' : ageFilter}
						</summary>
						{openDetails === 'age' && (
							<div className='flex flex-col absolute bg-[#202020] z-4 w-[180px]'>
								<button
									onClick={() => {
										setAgeFilter('18+')
										setOpenDetails(null)
					...
					<button
						...
						className={`cursor-pointer ${
							booleanFilter ? 'text-[#00A878]' : 'text-gray-400/70'
						}`}
					>
