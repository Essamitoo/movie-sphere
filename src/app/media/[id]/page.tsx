...
const MediaPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!data || !id) return
        setIsInViews(data.views?.includes(id) || false)
        setIsInFavorite(data.favorites?.includes(id) || false)
        setIsInList(data.list?.includes(id) || false)
    }, [data, id])

    const toggleFavorite = () => {
        if (!isInFavorite) {
            setData({ favorites: [...(data?.favorites || []), id] })
        } else {
            setData({ favorites: data?.favorites?.filter(item => item !== id) || [] })
        }
    }
    ...
    return (
        <div className='max-h-max min-h-lvh pb-5 text-white flex flex-col items-center gap-6'>
            ...
            <div className='absolute top-[240px] left-8 flex gap-2'>
                <button onClick={() => setOpen(true)} className='bg-gray-300 font-medium text-black rounded-lg w-[150px] flex gap-1 p-2 hover:scale-105'>
                    <FaPlay size={15} /> TRAILER
                </button>
                <button onClick={addList} className='border-2 border-gray-300 font-medium text-white text-sm rounded-lg w-[150px] flex gap-1 p-2 hover:scale-105'>
            ...
            <div className='w-[90%] border border-black h-[300px] mt-10 grid grid-cols-[20%_50%_30%] rounded-lg'>
                ...
                <div className='h-full text-black text-center flex flex-col justify-center gap-2'>
                    ...
                    <p className='text-gray-700 text-sm'>Fecha de estreno: {formatearFecha(mediaInfo?.releaseDate)}</p>
                    <p className='text-md'>Duración: {mediaInfo?.duration} min</p>
                    <p className='text-md'>Serie: <span className='font-semibold'>{mediaInfo?.series}</span></p>
                ...
                <div className='text-black flex flex-col justify-around items-center'>
                    <button onClick={toggleFavorite} className={`w-10 h-10 rounded-full border flex items-center justify-center ${isInFavorite ? 'text-[#00A878] bg-black' : 'text-white bg-gray-700/70'}`}>
                        <FaHeart size={20} />
                    </button>
                </div>
            </div>
            ...
            <Reparto />
            <h2 className='text-black font-semibold text-2xl'>Tu opinión nos importa, compártela aquí.</h2>
            ...
            <div className='flex flex-col justify-center gap-4'>
                <p className='text-black text-xl font-semibold'>Películas y series similares</p>
                <div className='grid grid-cols-4 gap-4'>
                    {mediaInfo?.suggestions?.map((item, index) => (
                        <img key={index} className='w-[180px] h-[250px] rounded-lg' src={item.img} alt='' />
                    ))}
                </div>
            </div>
        </div>
